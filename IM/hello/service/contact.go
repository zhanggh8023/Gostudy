...
func (service *ContactService) AddFriend(
	...
	//查询是否已经是好友
	// 条件的链式操作
	_, _ = DbEngin.Where("ownerid = ?", userid).
		And("dstid = ?", dstid).
...
func (service *ContactService) SearchFriend(userId int64) []model.User {
	conconts := make([]model.Contact, 0)
	objIds := make([]int64, 0)
	_ = DbEngin.Where("ownerid = ? and cate = ?", userId, model.CONCAT_CATE_USER).Find(&conconts)
	for _, v := range conconts {
	...
	_ = DbEngin.In("id", objIds).Find(&coms)
	return coms
}
